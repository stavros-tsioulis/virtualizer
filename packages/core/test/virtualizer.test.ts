...
describe("Virtualizer", () => {
  ...
  it.skip("should properly virtualize items", async () => {
    ...
    const [
      ...
      fourth,
      fifth
    ] = (virtualizer.ctx.target?.children ?? []) as HTMLElement[]
  ...
  it("should integrate new elements added after activation", async () => {
    ...
    assert.ok(newElement.classList.contains("virtualizer-item"), "New element should have a marker that it is virtualized")
    ...
    assert.ok(newElement2.classList.contains("virtualizer-item"), "New element should have a marker that it is virtualized")
  ...
  it("should trigger events appropriately", async () => {
    ...
    let addedCount = 0, updatedCount = 0, hiddenCount = 0, visibleCount = 0, renderedCount = 0;
    ...
    virtualizer.on("added", () => { addedCount++; });
    virtualizer.on("updated", () => { updatedCount++; });
    virtualizer.on("hidden", () => { hiddenCount++; });
    virtualizer.on("visible", () => { visibleCount++; });
    virtualizer.on("rendered", () => { renderedCount++; });
